...
def convert_rpg_to_java(rpg_code):
     prompt = 'Translate the following Assembly Language code to Java.java code should follow Object-oriented programming format, with proper comments and import related packages and main fuction should be always included:\n\n' + rpg_code + '\n\nJava code:'
...
def generateJava(businessLogic): 
  ...
  java_code_schema = ResponseSchema(name='python_code',description='This is the micropython script generated compatible with latest python version and which can run on arduino board converted to a correct json string without python backticks with new line replaced with \\n.')
  java_code_description_schema = ResponseSchema(name='python_code_description',description='This is the description of the python code generated')
